...
__global__
void evaluate(float* genes, int foods[][2], int* fitness_score, int GENOME_LENGTH) {
	/**
	...
	*/
	...
	__syncthreads();
	printf("Organism intialized %d\n", blockDim.x);
	if(THREAD_ID == 0) {
		do
		{
			if(foodEaten) {
				food_pos[0] = foods[fi][0];
				food_pos[1] = foods[fi][1];
				fi++;
				foodEaten = false; 
			}
			int head[2];
...
int main() 
{
	...
	*/
	...
	int* h_fitness_score;
	int* d_fitness_score;
	h_fitness_score = (int*)malloc(sizeof(int)*POPULATION_SIZE);
	...
	for(int i = 0; i < NUM_GENERATIONS; i++) {
		...
		*/
		...
		evaluate<<<blocks, threads, sizeof(float)*GENOME_LENGTH>>>(d_organism, foods, d_fitness_score, GENOME_LENGTH);
		cout << "Evaluration completed" << endl;
		/* Copy device memory to host */
		CUDA_CALL(cudaMemcpy(organism, d_organism, size1, cudaMemcpyDeviceToHost));
		CUDA_CALL(cudaMemcpy(h_fitness_score, d_fitness_score, sizeof(int)*POPULATION_SIZE, cudaMemcpyDeviceToHost));
